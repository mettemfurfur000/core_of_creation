...
painter::painter()
{
	...
	D.normal_fonts.init(4,18);
	D.normal_fonts.automatic_load("fonts\\");
...
{
	//special edition!
	//i cant do this now, maybe i can do this later?
	
	if(D.edit_mode)
	{
		D.edit_move_object_in_menu(Menu.menu_box,Text.text_box);
		D.edit_text_in_menu(Menu.menu_box,Text);
	}
...
void painter::button_draw(menu &Menu,button &Button)
{
	...
	if(D.edit_mode)
	{
		D.edit_move_object_in_menu(Menu.menu_box,Button.text_part.text_box);
	}
	//
	
	this->D.button_update(Button,t);
...
{
	for(int i = 0;i<D.menustack_size;i++)
	{
		if(D.menustack[i]) menu_draw(*D.menustack[i]);
	}
}